...
Use descriptive names for objects in code, e.g., `model_results` rather than `out` or `kangaroo_data` rather than `k`. However, do note prefer overly long names (e.g., three words is better than four or five).
...
### Accessing the Stanford GitHub Organization

Our GitHub organization is housed in the larger [Stanford-managed GitHub enterprise account](https://uit.stanford.edu/service/github). This means you will need to authenticate via your Stanford account, such as through your login or your [Cardinal Key](https://uit.stanford.edu/service/cardinalkey). You will also need to authenticate for private GitHub repos.

For most purposes, the simplest way to authenticate is with the [gh command line tool](https://cli.github.com/), running this in the terminal:

```bash
gh auth login
```

Our Nero instances come with gh pre-installed. Sherlock also has gh but you must load it as a module (`ml gh`) first. You may also need to use an SSH key rather than https when you login.

For SSH keys, you need to [allow the key to authenticate via your Stanford account](https://docs.github.com/en/enterprise-cloud@latest/authentication/authenticating-with-single-sign-on/authorizing-an-ssh-key-for-use-with-single-sign-on).
